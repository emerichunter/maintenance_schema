...
GROUP BY child_table, child_column, parent_table, parent_column, schema_name 
HAVING COUNT(*)>1
...
FROM (
    SELECT pg_sui.schemaname as schema_name, pg_sui.indexrelname,i.indexrelid,indisprimary,  i.indexrelid::regclass AS idx, (indrelid::text ||E'\n'|| indclass::text ||E'\n'|| indkey::text ||E'\n'||
                                         COALESCE(indexprs::text,'')||E'\n' || COALESCE(indpred::text,'')) AS KEY
    FROM pg_index i
...
-- Statements to DROP redundant indexes : TODO
/*
...
GRANT USAGE ON SCHEMA maintenance_schema TO postgres ;
GRANT SELECT ON ALL TABLES IN SCHEMA maintenance_schema TO postgres ;
ALTER ROLE postgres SET search_path TO   "$user", public, maintenance_schema;
ALTER SCHEMA maintenance_schema OWNER TO postgres;
