...
-- 6. Top 10 Time & IO Consumers
-- Compatible with PG 13+ (total_exec_time) and PG < 13 (total_time)
DO $$
DECLARE
    v_ver integer;
BEGIN
    v_ver := current_setting('server_version_num')::integer;
    
    IF v_ver >= 130000 THEN
        EXECUTE '
    ...
    ELSE
        -- For older versions using total_time
        EXECUTE '
            CREATE OR REPLACE VIEW maintenance_schema.rpt_pss_top_time_io AS
            SELECT
                substring(query, 1, 100) AS short_query, calls,
                ...
                round((blk_read_time + blk_write_time)::numeric, 2) AS io_time_ms,
                round((100 * (blk_read_time + blk_write_time) / NULLIF(total_time, 0))::numeric, 2) AS io_percent
            FROM pg_stat_statements
            ORDER BY total_time DESC LIMIT 10;';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Could not create rpt_pss_top_time_io: %', SQLERRM;
END
$$;
...
-- 8. Real-Time Blocking Tree
-- Requires pg_blocking_pids (PG 9.6+)
DO $$
DECLARE
    v_ver integer;
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Skipping rpt_activity_blocking_tree: Requires PostgreSQL 9.6+';
    END IF;
END
$$;
...
-- 14. Replication Slots Health
-- Requires pg_replication_slots (PG 9.4+)
DO $$
DECLARE
    v_ver integer;
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Skipping rpt_replication_slots_health: Requires PostgreSQL 9.4+';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        -- Fallback for versions where pg_wal_lsn_diff might be pg_xlog_location_diff (pre-10)
        IF v_ver < 100000 AND v_ver >= 90400 THEN
             EXECUTE '
                CREATE OR REPLACE VIEW maintenance_schema.rpt_replication_slots_health AS
                SELECT 
                    slot_name, plugin, active,
                    CASE WHEN NOT active THEN ''DANGER: Inactive Slot (Holding WAL)'' ELSE ''OK'' END AS status,
                    pg_size_pretty(pg_xlog_location_diff(pg_current_xlog_location(), restart_lsn)) AS retained_wal_size,
                    restart_lsn
                FROM pg_replication_slots
                ORDER BY active ASC, pg_xlog_location_diff(pg_current_xlog_location(), restart_lsn) DESC;';
        END IF;
END
$$;
